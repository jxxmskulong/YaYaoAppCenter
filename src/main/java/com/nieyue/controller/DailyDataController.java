...
public class DailyDataController {
	...
	public @ResponseBody StateResultList browsePagingDailyData(
			@RequestParam(value="recordDate",required=false)Date recordDate,
			...
			list= dailyDataService.browsePagingDailyData(recordDate,appId,acountId,pageNum, pageSize, orderName, orderWay);
			if(list.size()>0){
	...
	public @ResponseBody int countAll(
			...
			@RequestParam(value="acountId",required=false)Integer acountId,
			HttpSession session)  {
		int count = dailyDataService.countAll(recordDate,appId,acountId);
		return count;
	}
	/**
	 * 每日数据单个加载
	...
	public @ResponseBody StateResultList loadDailyDataByParams(
			...
			HttpSession session)  {
		List<DailyData> list = new ArrayList<DailyData>();
		DailyData dailyData = dailyDataService.loadDailyData(dailyDataId,recordDate,appId,acountId);
		if(dailyData!=null &&!dailyData.equals("")){
			list.add(dailyData);
			return ResultUtil.getSlefSRSuccessList(list);
		}else{
